...
if(isset($_GET['bbecid'])) {
	$sql_query = "SELECT display_name FROM wp_users WHERE id = (SELECT user_id FROM wp_cimy_uef_data WHERE field_id = \"9\" AND VALUE = \"" . $_GET['bbecid'] . "\");";
	$sql = $wpdb->get_results($sql_query);
	$parsed_sql = json_decode(json_encode($sql), True);
	$user_id = $parsed_sql[0]['display_name'];
	echo "<div><span id=\"name\">" . $user_id . "</span></div>";
}
...
if(isset($_GET['firstname']) && isset($_GET['lastname'])) {
	...
	<div style="max-width:1280px;margin:auto;">
		<div class="jfj-card jfj-person-card col-xs-12">
			<div class="col-xs-3">
				<a href="http://dev01.jewsforjesus.org/about-jews-for-jesus/staff/' . strtolower($user_meta_data[first_name][0])  . '-' . strtolower($user_meta_data[last_name][0]) . '/ "><div class="jfj-person-card-image" id="image" style=\'background-image:url("http://dev01.jewsforjesus.org/wp-content/uploads/' . strtolower($user_meta_data[first_name][0])  . '-' . strtolower($user_meta_data[last_name][0]) . '.jpg");\'>
				</div></a>
			</div>
			<div class="jfj-person-card-info col-xs-9">';
				if($user_meta_data[first_name][0] == $user_meta_data[last_name][0]) {
				echo '<a class="jfj-person-card-name" href="http://dev01.jewsforjesus.org/about-jews-for-jesus/staff/' . strtolower($user_meta_data[first_name][0])  . '-' . strtolower($user_meta_data[last_name][0]) . '/ "><h2>' . $user_meta_data[first_name][0] . '</h2></a>'; 
				} else {
				echo '<a class="jfj-person-card-name" href="http://dev01.jewsforjesus.org/about-jews-for-jesus/staff/' . strtolower($user_meta_data[first_name][0])  . '-' . strtolower($user_meta_data[last_name][0]) . '/ "><h2>' . $user_meta_data[first_name][0] . ' ' . $user_meta_data[last_name][0] . '</h2></a>'; 
				}
				echo '<p class="jfj-person-card-bio id" id="bio">' . $user_meta_data[description][0] . '</p>'; 

				if($user_meta_data[first_name][0] != ("Berlin" || "Budapest" || "New" || "Boston" || "Odessa" || "Kiev" || "Kharkov" || "Dnepropetrovsk" || "London" || "Moscow" || "Johannesburg" || "Geneva" || "Tel" || "Essen" || "Paris" || "Toronto" || "Montreal" || "Brazil" || "Sydney" || "Chicago" || "Washington" || "Brooklyn" || "San" || "Los" || "Phoenix" || "Fort")) {
