	...
	switch($deputation[0][2]){
		...
		case 'Message from the Word':
			...
			if($deputation[0][33] != ''){
				...
				$posterDetails['information'] = '<nl>'.$deputation[0][2].'</nl><nl>'.$deputation[0][33].'</nl><nl><sub>with</sub></nl><nl>'.$deputation[0][15].'</nl><nl>'.getSchedule(simplexml_load_string($deputation[0][11])).'</nl><nl>'.$deputation[0][5].'</nl><nl>'.preg_replace("/\r\n|\r|\n/","</nl><nl>",$deputation[0][7]).'</nl><nl>'.$deputation[0][8].'</nl><nl>'.$deputation[0][6].'</nl>';
